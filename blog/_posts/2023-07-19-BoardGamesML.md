---
layout: post
title: Warum dich der Computer in Brettspielen (fast) immer schlagen wird (Fortsetzung)
...
Jetzt starten wir die eigentliche Trainingsphase. Im Allgemeinen sieht diese so aus, dass wir die KI viele Spiele gegen Gegner spielen lassen.
...
$r_{t}$ beschreibt die Belohnung. Wenn der gewählte Zustand unmittelbar einen Gewinn auslöst, setzen wir hierfür 1 ein. Wenn der Zug eine unmittelbare Niederlage auslöst, wird -1 eingesetzt.
...
Das Training wird jetzt so lange durchgeführt, bis sich die Werte in der Q-Tabelle nur noch wenig ändern. Wie lange das dauert, hängt stark von der Größe der Tabelle ab.
...
In dem folgenden Bild ist ein neuronales Netz abgebildet.
...
um die Differenz zu minimieren. Wie das genau und mathematisch korrekt funktioniert, werde ich in einem zukünftigen Kapitel behandeln.
...
Im weiteren Verlauf werde ich dieses neuronale Netz Q-Netz nennen. Die Eingabe in unser Q-Netz ist immer ein Spielzustand. Ein Spielzustand besteht immer aus 7x6 Felder.
Das bedeutet, dass wir 42 Neuronen als Input Layer benötigen.\
Als Ausgabe wollen wir die Q-Werte der einzelnen Aktionen bekommen. Wir haben maximal 7 Aktionen. D.h. wir benötigen 7 Neuronen im Output Layer.
Wie die Verborgene Schicht aufgebaut ist, spielt in diesem Beispiel keine Rolle. Neugierige verweise ich hier gerne auf die [Implementierung](https://github.com/omegacf/OmegaCF-client/blob/dev/include/ml/network.hpp) (tatsächlich habe ich auch ein Convolution Neuronal Network verwendet).

Wie trainieren wir nun dieses Q-Netz? Wie zuvor erwähnt, ist hier vor allem die Differenz zwischen der Ausgabe des Q-Netzes und der erwünschten Ausgabe von Bedeutung.
Die Differenz lässt sich mit folgender Formel berechnen:

$$
\text{Loss} = (r_{t} + \max_{a}Q(s_{t+1}, a) - Q(s,a))^2
$$

Das Q-Netz liefert perfekte Ergebnisse, wenn die Loss-Funktion null zurückgibt. Diese Funktion wird während des Trainings für den bereits angesprochenen Rückwärtsdurchlauf verwendet.\
Wenn man das ganze tatsächlich implementieren will, sollte ein Target-Network (eine Kopie des Q-Netzes) verwendet werden, um $\max_{a}Q(s_{t+1}, a)$ zu berechnen.

Jetzt können wir dieses Q-Netz anstatt der Q-Tabelle verwenden. Wenn die KI am Zug ist, übergibt sie den aktuellen Zustand an das Q-Netz. Das Q-Netz gibt die Q-Werte der einzelnen Aktionen aus, 
ähnlich zur Q-Tabelle. Die KI muss jetzt nur noch die Aktion mit dem höchsten Q-Wert auswählen und diese auch spielen.

## Zusammenfassung
Wir haben uns in diesem Artikel angesehen, wie wir mit Q-Learning und Deep-Q-Learning das Spiel Vier Gewinnt spielen können. Auch merkt man schnell, dass es viele Parameter, also Stellschrauben,
gibt, die dafür justiert werden müssen. Es gehört also viel Trial-and-Error dazu, um mit diesen Methoden eine zuverlässige KI zu erschaffen.\
Ich habe das mal versucht und meinen [Code](https://github.com/omegacf/OmegaCF-client) dazu natürlich veröffentlicht.
